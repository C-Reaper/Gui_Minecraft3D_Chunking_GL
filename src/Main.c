...
GLuint loadTexture(const char* path) {
	...
	Sprite tex = Sprite_Load_BGR((char*)path);
    if (tex.img) {
...
void GL_Init(){
	...
	Sprite tex = Sprite_Load_BGR("./assets/Std_Atlas.png");
    if (tex.img) {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, (GLsizei)tex.w,(GLsizei)tex.h, 0, GL_RGBA, GL_UNSIGNED_BYTE, (const void*)tex.img);
        glGenerateMipmap(GL_TEXTURE_2D);
